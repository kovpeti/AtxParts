...
interface iaxtparts
{
	...
	public function FormRender_Head($params);
	public function FormRender_BodyTag($params);
	public function FormRender_Tabs($params);
	public function FormRender_TopPanel($params);
	public function FormRender_TopPanel_Login($params);
	public function FormRender_BottomPanel($params);
	public function FormRender_BottomPanel_Login($params);
	public function FormRender_PopClose();
	public function FormRender_Grid($totalwidth = 960, $numcells = 24);
}
...
class axtparts implements iaxtparts
{
	...
	*/
	...
	/**
	 * @return void.
	 * @param array $params. Contains the various parameters required for the rendering of the head section
	 * Includes: array["meta"], "title", "icon", array["css], array["jscript_file"], array["jscript_local"]
	 * @desc Outputs the head section of a form using the parameters given.
	 */
	public function
	FormRender_Head($params)
	{
		print "<head>\n";
		...
		if (isset($params["icon"]))
			print "<link rel=\"shortcut icon\" type=\"image/x-icon\" href=\"".$params["icon"]."\" />\n";
	
		if (isset($params["css"]))
		{
			$n = count($params["css"]);
			for ($i = 0; $i < $n; $i++)
				print "<link rel=\"stylesheet\" type=\"text/css\" href=\"".$params["css"][$i]."\" />\n";
		}
	
		if (isset($params["jscript_file"]))
		{
			$n = count($params["jscript_file"]);
			for ($i = 0; $i < $n; $i++)
				print "<script type=\"text/javascript\" language=\"javascript\" src=\"".$params["jscript_file"][$i]."\" ></script>\n";
		}
	
		if (isset($params["jscript_local"]))
		{
			$n = count($params["jscript_local"]);
			for ($i = 0; $i < $n; $i++)
				print "<script type=\"text/javascript\" language=\"javascript\">\n".$params["jscript_local"][$i]."</script>\n";
		}
	...
	/**
	 * @return void.
	 * @param array $params. Contains the various parameters required for the rendering of the body tag
	 * Includes: array["onload"]
	 * @desc Outputs the body tag using the parameters given.
	 */
	public function
	FormRender_BodyTag($params)
	{
		print "<body";
		if (isset($params["onload"]))
		{
			$n = count($params["onload"]);
			if ($n > 0)
			{
				print " onload='";
				for ($i = 0; $i < $n; $i++)
					print $params["onload"][$i].";";
				print "'";
			}
		}
	...
	* @return void.
	 ...
	 */
	 ...
	 FormRender_Tabs($params)
	 {
		...
		print "<ul class=\"tabnames\">\n";
		if ($nt > 0)
		{
			print "<li class=\"pad\"></li>\n";
			...
			foreach ($tabs as $tablabel => $tabset)
			{
				...
				if (isset($tabset["onclick"]))
					$tabonclick = $tabset["onclick"];
				else
					$tabonclick = false;
				...
				if (isset($tabset["tabpriv"]))
				{
					...
					if ($this->SessionMePrivilegeBit($tabpriv) === true)
					{
						...
						if ($tabhref !== false)
						{
							print "<a href=\"".$tabhref."\" ";
							if ($tabonclick !== false)
								print "onclick='".$tabonclick."' ";
							print ">";
						}
						
						print "<span class=\"tabtext\">".$tablabel."</span>";
						if ($tabhref !== false)
							print "</a>";
	...
	/**
	* @return void.
	 * @param array $params. Contains the various parameters required for the rendering of the top panel
	 * Includes: ["logoimgurl"], ["siteheading"], array["info"]{'fn'], ['mi'], ['ln'], ['region'], ['udh'], ["statusimgurl"],
	 * array["buttons"]["search"]['imgurl'], ['href'], ['title'], ['onclick'], ["help"], ["info"], ["newuser"], ["logout"]
	 * @desc Outputs the top panel section using the parameters given.
	 * Depends on the correct css and javascript files having been included. Also requires the <div class="formpanel">
	 * to be already in place (provides the container scope and background)
	 */
	 public function
	 FormRender_TopPanel($params)
	 {
		print "<section class=\"toppanel\">\n";
		print "<table class=\"toppanel\">\n";
		print "<tr>\n";
	
		// System Heading
		print "<td class=\"toppanelcell_lt\">\n";
		if (isset($params["siteheading"]))
			print "<span class=\"engtitle\">".htmlentities($params["siteheading"])."</span><br/>\n";
		if ($params["formtitle"])
			print "<span class=\"formtitle\">".htmlentities($params["formtitle"])."</span><br/>\n";
		print "</td>\n";
	
		// buttons and username
		print "<td class=\"toppanelcell_rt\">\n";
		if (isset($params["username"]))
			print "<span class=\"username\">".htmlentities($params["username"])."</span><br/>\n";
		else
			print "<span class=\"username\">&nbsp;</span><br/>\n";
		
		if (isset($params["buttons"]))
		{
			if (isset($params["buttons"]["logout"]))
			{
				if (isset($params["buttons"]["logout"]["btnname"]) && isset($params["buttons"]["logout"]["onclick"]))
				{
					$btnname = $params["buttons"]["logout"]["btnname"];
					$onclick = $params["buttons"]["logout"]["onclick"];
					
					if (isset($params["buttons"]["logout"]["title"]))
						$title = $params["buttons"]["logout"]["title"];
					else
						$title = "Log Out";
					
					if (isset($params["buttons"]["logout"]["label"]))
						$label = $params["buttons"]["logout"]["label"];
					else
						$label = "Logout";

					print "<input type=\"button\" class=\"btntext\" name=\"".$btnname."\" value=\"".htmlentities($label)."\" title=\"".htmlentities($title)."\" onclick=\"".$onclick."\" />\n";
				}
			}
		}
		print "</td>\n";
	 ...
	 /**
	  * @return void.
	  * @param array $params. Contains the various parameters required for the rendering of the top panel for the loginform
	  * Includes: ["logoimgurl"], ["logohref"]
	  * @desc Outputs the login form top panel section using the parameters given.
	  * Depends on the correct css and javascript files having been included. Also requires the <div class="loginpanel">
	  * to be already in place (provides the container scope and background)
	  */
	public function
	FormRender_TopPanel_Login($params)
	{
		print "<section class=\"toppanel\">\n";
		print "<table class=\"toppanel\">\n";
		print "<tr>\n";

		// logo - 188w x 80h image file
		print "<td class=\"toplogo\">\n";
		if (isset($params["logoimgurl"]))
		{
			if (isset($params["logohref"]))
				print "<a href=\"".$params["logohref"]."\"><img src=\"".$params["logoimgurl"]."\" height=\"80\" width=\"188\" border=\"0\" /></a>\n";
			else
				print "<img src=\"".$params["logoimgurl"]."\" height=\"80\" width=\"188\" border=\"0\" />\n";
		}
		print "</td>\n";
		print "</tr>\n";
		print "</table>\n";
	 ...
	 /**
	  * @return void.
	  * @param array $params. Contains the various parameters required for the rendering of the bottom form panel
	  * Includes: ["authentxlogourl"], ["serverid"], ["entstatus"], {"edipi"], ["endform"]
	  * array["buttons"]["submit"]['type'], ['class'], ['title'], ['onclick'], ['value'], ["reset"], ["logs"]
	  * Additional buttons may be included as required, provided the 'name' (index) is different.
	  * @desc Outputs the bottom form panel section using the parameters given.
	  * Depends on the correct css and javascript files having been included. Also requires the <div class="formpanel">
	  * to be already in place (provides the container scope and background)
	  */
	public function
	FormRender_BottomPanel($params)
	{
		print "<section class=\"bottompanel\">\n";
		print "<table class=\"bottompanel\">\n";
		print "<tr>\n";

		print "<td class=\"bottominfo\">\n";
		print "<span class=\"bottominfogrey\" id=\"remainingsessiontime\"></span>\n";
		print "</td>\n";
		
		// Branding
		print "<td class=\"bottomlogo\">\n";
		if (isset($params["branding"]))
			print "<span class=\"footertext\">".$params["branding"]."</span>\n";
		print "</td>\n";
	 ...
	 /**
	  * @return void.
	  * @param array $params. Contains the various parameters required for the rendering of the bottom login form panel
	  * Includes: ["authentxlogourl"], ["serverid"], ["siteheading"]
	  * @desc Outputs the bottom login form panel section using the parameters given.
	  * Depends on the correct css and javascript files having been included. Also requires the <div class="loginpanel">
	  * to be already in place (provides the container scope and background)
	  */
	 public function
	 FormRender_BottomPanel_Login($params)
	{
		print "<section class=\"bottompanel\">\n";
		print "<table class=\"bottompanel\">\n";
		print "<tr>\n";
		...
		print "<td class=\"bottominfo\">\n";
		if (isset($params["siteheading"]))
			print "<span class=\"engtitle\">".htmlentities($params["siteheading"])."</span>\n";
		print "</td>\n";
	 ...
	 /**
	  * @return void.
	  * @desc Outputs the right-justified close button for a popup
	  */
	public function
	FormRender_PopClose()
	{
		print "<table class=\"popclose\">\n";
		print "<tr>\n";
		print "<td class=\"contentcell_rt\">\n";
		print "<input type=\"button\" name=\"close\" class=\"btntext\" value=\"Close\" onclick=\"javascript:window.close()\" />\n";
		print "</td></tr>\n";
		print "</table>\n";
	}
	 ...
	 /**
	  * @return void.
	  * @param $totalwidth. The total width of the table in px (default 960px)
	  * @param $numcells. The number of cells to divide the table row into (default 24)
	  * @desc Outputs the table grid cell structure
	  */
	public function
	FormRender_Grid($totalwidth = 960, $numcells = 24)
	{
		$cellwidth = intval($totalwidth/$numcells);
		for ($i = 0; $i < $numcells; $i++)
			print "<col width=\"".$cellwidth."\" />";
